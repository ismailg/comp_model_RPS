...
To measure transfer between games, we again focus on participants' scores in the initial 5 rounds after the first round (rounds 2-6) of each game and only during the first time they interact with an opponent in a game (see Figure \ref{fig:exp2-early-score-by-opp}). We estimated a linear mixed-effects model with the early round scores as the dependent variable, and Condition (level-1 first, level-2 first), Opponent type (level-1 or level-2) and Game (RPS, FWG, Shootout) as independent variables, including all main effects and interactions. The model also included random intercepts for participants. We found a main effect of Game (\(F(2,240) = 6.57\), \(p = .002\)). \textbf{The effect of game reflects early scores on FWG being significantly higher than on RPS (\(t(240) = 3.01\), \(p = .005\)) as well as scores on shootout being significantly higher than RPS (\(t(240) = 3.25\), \(p = .003\)). We also found an interaction effect between Game and Opponent type (\(F(2,240) = 3.34\), \(p = .037\)). Post-hoc pairwise comparisons (applying Holm correction for multiple comparisons) show that the difference between early scores in RPS and shootout was mostly driven by participants facing level-1 players ((\(t(240) = 4.12\), \(p < .001\))). No signifcant differences were found between opponents for other games, or between games when facing the same opponent.}

\textbf{MS: What do these effects indicate? Report post-hoc comparisons between games, and e.g.~differences between opponents for each game, here. The later tests against 0 are not adequate to understand the effects here.}
...
where \(S_{p}(t)\) quantifies how the opponent deviates from past play. It is calculated in turn through the cumulative history of play, across strategies k, as \(h^{k}(t)\):
...
\textbf{MS: why is there a superscript in \(h^k\) and \(r^k\)? }
\textbf{IG : Because they depend on strategy k}

\textbf{MS: And is the indicator variable \(I(o^k = o_{t})\)?}
\textbf{IG: Yes, corrected}

\textbf{And is \(\delta_a(t)\) always 0 or 1?}
\textbf{IG: yes. Pls refer to: Camerer, C. F., \& Ho, T. H. (2015). Behavioral game theory experiments and modeling. Handbook of game theory with economic applications, 4, 517-573.}

For more details on the self-tuning EWA model, we refer the reader to (Ho et al., 2004). As in the RL model above, actions are chosen based on a softmax decision rule:
