...
```{r exp1-3factor-plot, fig.cap = "Boxplots of scores by game, block and condition", fig.align = "center"}
...
```{r exp1-score-by-opp, fig.cap = "(ref:figure4-caption)", fig.align ='center'}

p2 <- ggerrorplot(exp1_dat2_6, x = "game.f", y = "early_score", group = 1, color="game.f", desc_stat = "mean_se",palette = c("#00AFBB", "#E7B800", "#FC4E07"), order = c("RPS", "FWG","Numbers"), ylab = "Early rounds score", xlab = "Games", legend.title = "Games")
...
```{r ANOVA early exp1, include = TRUE, warning=FALSE}
...
(ref:figure7-caption) Mean and standard error of early scores by game and opponent for experiment 2
```{r exp2-early-scores, fig.cap="(ref:figure7-caption)", fig.align="center"}
...
In experiment 1, we fit a total of 5 models: A baseline model assuming random play (Nash),  Bayesian Cognitive Hierarchy model allowing transfer betwen games (BCH_BT) and another with no transfer between games (BCH_NBT), as well as a Q-learning model with state space consisting of last round play (QL_states), and finally a self tuning EWA model with the same state space (STEWA_states). In experiment 2, because participants were interacting with each opponent twice within each game, we allowed models to transfer learning within games between the first and second interactions. Therefore on top of the two BCH models mentionned above (BCH_BT and BCH_NBT), we also allow the Q-learning model with states to either have the ability to transfer learning between the first and second interaction (QLS_Tr) or not (QLS_NT). Likewise, we fit both a self tuning EWA model with transfer between stages of the same game (STEWA_Tr) or without transfer (STEWA_NT). Counting the base model wiht random play (Nash) we therefore fit a total of 8 models in experiment 2. 
...
exp2_comp_table <- table(exp2_comp_results[, "condition"],c("Nash","BCH BT","BCH NBT", "BCH NT", "QLS NT","QLS Tr","STEWA NT","STEWA Tr")[apply(exp2_comp_results[,c("Random_BIC","Distinct_game_BIC","Distinct_stage_BIC","Bayes_distinct_no","QL_states_BIC","QLS_within_Tr_BIC","STEWA_NT_BIC","STEWA_Tr_BIC")],1,which.min)])
...
for(id in unique(dat_exp2$human_id)) {
  ...
  best_model <- c("Nash","BCH BT","BCH NBT", "BCH NT", "QLS NT","QLS Tr","STEWA NT","STEWA Tr")[apply(id_results[,c("Random_BIC","Distinct_game_BIC","Distinct_stage_BIC","Bayes_distinct_no","QL_states_BIC","QLS_within_Tr_BIC","STEWA_NT_BIC","STEWA_Tr_BIC")],1,which.min)]
...
colnames(exp2_BIC_weights) <- c("ID","Nash","BCH BT","BCH NBT", "BCH NT", "QLS NT","QLS Tr","STEWA NT","STEWA Tr")
...
ggplot(data_long2,aes(x=round,y=probability,colour=strategy)) +
  ...
  scale_colour_discrete(name = "Strategy", labels = c( "Nash", "Q_Learning + within Tr", "BCH + Tr"))
