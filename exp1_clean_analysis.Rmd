...
```{r}
library(afex)
mod_exp1 <- fixed( block_score ~ game.f*block*condition + Error(human_id/(game.f*block)), dat2)
summary(a1)
```
...
#### EARLY TRIALS - LEARNING TRANSFER !!!! 
```{r}
...
# Mixed effects Model: Random intercecept for participants
...
contrast(means.int2, list(L1vL2forG2 = c(0,1,0,0,-1,0), L1vL2forG3=c(0,0,1,0,0,-1)))
                                 # levels      c(1,-1,0,1,-1,0)
                                 # games       c(1,1,1,-1,-1,-1)

# Interaction contrast

contrast(means.int2, list(G1vG2byLevel = c(1,-1,0,-1,1,0), G2vG3byLevel6=c(0,1,-1,0,-1,1)))

         
# Plot interactions for early rounds, condition by game
