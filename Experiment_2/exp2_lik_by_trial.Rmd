...
for(id in unique(dat$human_id)) {
  ...
  # QLS likelihoods 
  pars_QLS <- unlist(filter(exp2_optim_results, ID == id)%>%select(QL_states_alpha,QL_states_Beta))
  tdat$QLS_lik <-  exp2_QL_states(pars_QLS,tdat,"likelihood_by_trial",0.0, FALSE)
  
  # QLS within Transfer likelihoods 
  pars_QLS_within <- unlist(filter(exp2_optim_results, ID == id)%>%select(QLS_within_Tr_alpha,QLS_within_Tr_Beta))
