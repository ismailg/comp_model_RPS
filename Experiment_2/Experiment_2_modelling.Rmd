...
# TOM_model <- function(par, data, opp_strategy_vec, return_value, within_transfer, btwn_transfer) {
#   
...
TOM_model <- function(par, data, opp_strategy_vec, return_value, within_transfer, btwn_transfer, Naive = FALSE) {
  ...
  # dat = data subset for one participant
  # opp_strategy_vec = vector of possible opponent strategies, model assumes humans restrict opp strategy space to vector   c("level0","level1","level2")
  # return = -2logLik 
  # or "likelihood_by_trial" vector  (for later plotting, etc)
  # btwn_transfer is a boolean. True if distribution of opponent strategies is kept across games. False otherwise. 
  ...
  # Load tables predicting oppponent model
  ...
  for(t in 1:nrow(data)) {
    ...
    if(data[t,"round"] == 1) {
      ...
      if (Naive) {
        ...
        # we are round 1 of stage 2, keep opp vector from round 20 of stage 1  for stage 3
            ...
            if (within_transfer){
              # hold the probs vector of opponent type learned in stage 1 in memory and save to temp.
              opp1_prior_temp <- opp_prior_vec
    ...
    # Rounds 2 to End of stage
      ...
      for (strategy in opp_strategy_vec) {
        ...
        # Given opponent action predicted, what would be human action best response for each opp strat
        br_hum[[k]] <- as.character(exp2_best_response(t_game,as.character(pred_opp)))
        
        # First we deal with easy case of unique predicted action ( multiple preds in shoot are separated by "/" sign)
        if (!grepl("/",pred_opp,fixed = TRUE)) {
           # Multiply prior by likelihood of observation to get posterior. Done here ot take advantage of For loop. 
          if(as.character(data[t,"a_action"]) == pred_opp) {
            lik_opp[k,t] <- (theta + (1-theta)/nopts)*opp_prior_vec[k]
        ...
        # Here we are in shootout and opponent predicted action is a vector of two possible moves (e.g "left/center")  
          ...
          if(grepl(as.character(data[t,"a_action"]), pred_opp,fixed = TRUE)) {
            # Assume each action has likelihood of 50% 
             lik_opp[k,t] <- (0.5*(theta + (1-theta)/nopts) + 0.5*((1-theta)/nopts)) *opp_prior_vec[k]
          } else { 
            lik_opp[k,t] <- ((1-theta)/nopts)*opp_prior_vec[k]
          }
        }
      ...
      # Get which opponent strategy human current actn maps to (i.e, if index = 0 then human current action is best response to level-0 comp_strat...)
      indices <- which(grepl(as.character(data[t,"h_action"]),br_hum,fixed = TRUE))
      not_pred_indx <- setdiff(c(1,2,3),indices)
      # cat(as.character(indices), "\n")

      # THEN likelihood of current human action is just sumproduct of priors and likelihood of each oppoennt action
      if (!grepl("/",br_hum,fixed = TRUE)) {
        lik_hum[t] <- sum(opp_prior_vec[indices])* (theta + (1-theta)/nopts) +    sum(opp_prior_vec[not_pred_indx])*(1-theta)/nopts
      } else {
        for (i in 1:3) {
          temp <- 0
          if (grepl("/",br_hum[i],fixed = TRUE)) {
            temp <- temp + 0.5*(theta + (1-theta)/nopts)*opp_prior_vec[i]
          } else{
            temp <- temp + 
          }
        }
      ...
      lik_opp[,t] <- lik_opp[,t]/sum(lik_opp[,t])
      opp_prior_vec <- lik_opp[,t]
    } # End of else ( from round 2 onwards)
    
    
  # for debugging uncomment the line below
  # cat(as.character(data[t,"a_action"]),lik_opp[,t],"\n")
  cat(as.character(br_hum),as.character(data[t,"h_action"]),lik_hum[t],"\n","\n")
...
```
## Fitting Bayesian model with between game transfer (as well as within)  
```{r, cache=TRUE}
exp2_BT <- list()
for(id in unique(dat_exp2$human_id)) {
  ...
  # TR_bayes_modelling[[id]] <- DEoptim(fn=naive_bayes, lower = c(0,0), upper = c(1,1), data=tdat, opp_strategy_vec = c("level0","level1","level2") ,return_value = "-2loglik", btwn_transfer = TRUE, control=list(trace = FALSE, parallelType=1,parVar = c("best_response")))

  exp2_BT[[id]] <- optim(c(0.1,0.1),fn=TOM_model,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", within_transfer = TRUE, btwn_transfer = TRUE, Naive = FALSE, lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
}
save(exp2_BT, file="exp2_BT.RData")

```

## Fitting ToM model with NO Between transfer (but with transfer within games)
```{r, cache=TRUE}
...
for(id in unique(dat_exp2$human_id)) {
  ...
  # exp2_NO_BT[[id]] <- DEoptim(fn=naive_bayes, lower = c(0.0,0.0), upper = c(1.0,1.0), data=tdat, opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", opp_mod_transfer = FALSE, control=list(trace = FALSE, parallelType=0,parVar = c("best_response")))

  exp2_NO_BT[[id]] <- optim(c(0.1,0.1),fn=TOM_model,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", within_transfer = TRUE, btwn_transfer = FALSE, Naive = FALSE, lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
}

save(exp2_NO_BT,file="exp2_NO_BT.RData")
```

## Fitting Bayesian model with NO Between or within transfer
```{r, cache=TRUE}
...
for(id in unique(dat_exp2$human_id)) {
  ...
  # exp2_NO_Tr[[id]] <- DEoptim(fn=naive_bayes, lower = c(0.0,0.0), upper = c(1.0,1.0), data=tdat, opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", opp_mod_transfer = FALSE, control=list(trace = FALSE, parallelType=0,parVar = c("best_response")))

  exp2_NO_Tr[[id]] <- optim(c(0.1,0.1),fn=TOM_model,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", within_transfer = FALSE , btwn_transfer = FALSE, Naive =FALSE, lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
}

save(exp2_NO_Tr,file="exp2_NO_Tr.RData")
```

#Fitting model where player does not distinguish between opponents 
```{r, cache=TRUE}
...
for(id in unique(dat_exp2$human_id)) {
  ...
  # exp2_Naive[[id]] <- DEoptim(fn=naive_bayes, lower = c(0.0,0.0), upper = c(1.0,1.0), data=tdat, opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", opp_mod_transfer = FALSE, control=list(trace = FALSE, parallelType=0,parVar = c("best_response")))

  exp2_Naive[[id]] <- optim(c(0.1,0.1),fn=TOM_model,gr = NULL, data=tdat,opp_strategy_vec = c("level0","level1","level2"),return_value = "-2loglik", within_transfer = FALSE , btwn_transfer = FALSE, Naive = TRUE, lower = c(0.01,0.01), upper = c(0.99,0.99), method="L-BFGS-B")
}

save(exp2_Naive,file="exp2_Naive.RData")
```
...
```{r}
exp2_QL_states(c(5,0.4),test_data, "-2loglik", 0.0, TRUE)
```
...
#                          "Naive_BIC" = exp2_Naive[[id]]$value+ 2*log(180),
                         ...
                         "theta_transfer" = exp2_BT[[id]]$par[1],
                         "theta_no_btw_tr" = exp2_NO_BT[[id]]$par[1],
                         "theta_no_tr" =  exp2_NO_Tr[[id]]$par[1],
                         "theta_naive" = exp2_Naive[[id]]$par[1],
                         # Epsilone is parameter showing cases where human deviates from predictions about opponent (lower better)
                         "eps_transfer" = exp2_BT[[id]]$par[2],
                         "eps_no_btwn_tr" = exp2_NO_BT[[id]]$par[2],
                         "eps_no_tr" = exp2_NO_Tr[[id]]$par[2],
                         "eps_naive" = exp2_Naive[[id]]$par[2],
...
```{r}
# WIth IG results for Bayesian models
# exp2_table_results <- table(exp2_results[, "condition"],c("random","TOM_BT","TOM_NBT", "TOM_NT","ToM_Naive", "QL_states","QLS_within","S_EWA","MBM")[apply(exp2_results[,c("Random_BIC","Btwn_TR_BIC","No_Btwn_Tr_BIC","No_Tr_BIC","Naive_BIC","QL_states_BIC","QLS_within_Tr_BIC","EWA_self_BIC","MBM2_BIC")],1,which.min)])
...
for(id in unique(dat_exp2$human_id)) {
  ...
  # IG BAYES MODELS 
  # min_BIC <- apply(id_results[,c("Random_BIC","Btwn_TR_BIC","No_Btwn_Tr_BIC","No_Tr_BIC","Naive_BIC", "QL_states_BIC","QLS_within_Tr_BIC","EWA_self_BIC","MBM2_BIC")],1,min)
  # 
  # best_model <- c("random","ToM_BT","ToM_NBT", "ToM_NT","ToM_Naive","QL_states","QLS_within","S_EWA", "MBM")[apply(id_results[,c("Random_BIC","Btwn_TR_BIC","No_Btwn_Tr_BIC","No_Tr_BIC","Naive_BIC","QL_states_BIC","QLS_within_Tr_BIC","EWA_self_BIC","MBM2_BIC")],1,which.min)]
  
  # MS BAYES MODELS
  min_BIC <- apply(id_results[,c("Random_BIC","Distinct_game_BIC","Distinct_stage_BIC","Bayes_distinct_no","Bayes_same_game", "QL_states_BIC","QLS_within_Tr_BIC","EWA_self_BIC","MBM2_BIC")],1,min)
  
  best_model <- c("random","ToM_BT","ToM_NBT", "ToM_NT","ToM_Naive","QL_states","QLS_within","S_EWA", "MBM")[apply(id_results[,c("Random_BIC","Distinct_game_BIC","Distinct_stage_BIC","Bayes_distinct_no","Bayes_same_game","QL_states_BIC","QLS_within_Tr_BIC","EWA_self_BIC","MBM2_BIC")],1,which.min)]
